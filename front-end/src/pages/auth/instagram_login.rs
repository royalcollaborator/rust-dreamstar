...
#[component]
pub fn InstagramLogin(code: String) -> Element {
    ...
    use_future(move || async move {
        ...
        show_loading();
        let response = match get_local_storage("enable_instagram") {
            Some(value) => {
                if value.to_string() == "true".to_string() {
                    ...
                    client
                        .put(format!(
                            "{}/api/v0/auth/instagram/callback?code={}",
                            SERVER_URL, code
                        ))
                        .headers(headers)
                        .send()
                    ...
                    match serde_json::from_str::<ApiResponse>(&body) {
                        Ok(response) if response.status == "success" => {
                            ...
                            notification_data.set({
                                ...
                                data.push(NotificationData {
                                    title: "".to_string(),
                                    content: "Instagram login success".to_string(),
                        ...
                        Ok(_) => {
                            notification_data.set({
                                ...
                                data.push(NotificationData {
                                    title: "".to_string(),
                                    content: "API returned a non-success status".to_string(),
