...
use crate::db_models::{ matches::Match, votes::Vote, user::User };

pub async fn voting_calc() {
    ...
    {
        //Todo this check can be in the db query like I was saying above.
        Ok(res) => {
                    ...
                    result
                        ...
                        .filter(
                            move |matches|
                                ...
                                (
                                    Utc::now() - Duration::hours(matches.voting_duration as i64)
                                ).timestamp()
                        )
                        .collect()
                }
            ...
            for data in filter_match {
                ...
                let a_avg_judge = if a_judges_score == 0 {
                    ...
                    ((a_judges_score as f64) /
                        ((a_judges_score as f64) + (b_judges_score as f64))) *
                        (100 as f64)
                };
                    ...
                    Ok(_) => {
                        ...
                        let winner_information = match
                            user_collection.find_one_and_update(
                                ...
                                doc! {
                                        "$addToSet": { "matches_won": data.match_id.to_string() },
                                        "$pullAll": { "matches_lost": [data.match_id.to_string()] },
                                        "$inc": {
                                            "votes_for": if winner_username.to_string() == data.a_camp_username.to_string() {
                                                a_official_score as i32
                                            } else {
                                                b_official_score as i32
                                            },
                                            "judge_votes": if winner_username.to_string() == data.a_camp_username.to_string() {
                                                a_judges_score as i32
                                            } else {
                                                b_judges_score as i32
                                            },
                                            "final_votes": if winner_username.to_string() == data.a_camp_username.to_string() {
                                                a_final_score as i32
                                            } else {
                                                b_final_score as i32
                                            },
                                        },
                        },
                                None
                        ...
                        let loser_information = match
                            user_collection.find_one_and_update(
                                ...
                                doc! {
                                        "$addToSet": { "matches_lost": data.match_id.to_string() },
                                        "$pullAll": { "matches_won": [data.match_id.to_string()] },
                                        "$inc": {
                                            "votes_for": if loser_username.to_string() == data.a_camp_username.to_string() {
                                                a_official_score as i32
                                            } else {
                                                b_official_score as i32
                                            },
                                            "judge_votes": if loser_username.to_string() == data.a_camp_username.to_string() {
                                                a_judges_score as i32
                                            } else {
                                                b_judges_score as i32
                                            },
                                            "final_votes": if loser_username.to_string() == data.a_camp_username.to_string() {
                                                a_final_score as i32
                                            } else {
                                                b_final_score as i32
                                            },
                        ...
                        let _ = send_email(
                            ...
                            "<p>Congratulations! Great Job.</p>".to_string()
                        );
                        // send to loser
                        let _ = send_email(
                            loser_information.username.to_string(),
